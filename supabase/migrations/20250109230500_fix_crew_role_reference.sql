-- ============================================================================
-- EMERGENCY FIX V6: Use correct role_id reference to crew_roles
-- 
-- ISSUE: project_event_roles.role_id has foreign key to crew_roles(id), not project_roles(id)
-- From 20250804170918_remote_commit.sql:
-- project_event_roles.role_id → crew_roles(id)
-- project_roles.role_id → crew_roles(id)
--
-- SOLUTION: Use project_roles.role_id (which contains crew_roles.id values)
-- ============================================================================
...
BEGIN
  ...
  -- ✅ FIXED: Use project_roles.role_id (which contains crew_roles.id values)
  -- This satisfies the foreign key constraint: project_event_roles.role_id → crew_roles.id
  INSERT INTO project_event_roles (
    project_id,
    event_id, 
    role_id,           -- References crew_roles.id
    crew_member_id,
    daily_rate,
    created_at,
    updated_at
  ...
  SELECT DISTINCT ON (pr.role_id)
    p_project_id,
    p_event_id,
    pr.role_id,        -- ✅ CORRECT: Use project_roles.role_id (contains crew_roles.id)
    pr.preferred_id,
    pr.daily_rate,
    NOW(),
    NOW()
  ...
  WHERE pr.project_id = p_project_id
    AND pr.variant_id = v_actual_variant_id
    AND pr.role_id IS NOT NULL  -- Ensure role_id exists
