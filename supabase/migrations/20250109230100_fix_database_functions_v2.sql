-- ============================================================================
-- EMERGENCY FIX V2: Correct Database Function Issues
-- 
-- Fixes:
-- 1. Equipment duplicate key constraint violation
-- 2. Crew role_id column name (not project_role_id)
-- ============================================================================

-- Fix sync_event_equipment_unified to handle duplicates properly
...
BEGIN
  ...
  -- ✅ FIXED: Delete existing equipment first to avoid constraint violation
  DELETE FROM project_event_equipment 
  WHERE event_id = p_event_id;
  
  -- ✅ FIXED: Use INSERT with ON CONFLICT to handle any remaining duplicates
...
-- ============================================================================
-- Fix sync_event_crew function with correct column names
-- ============================================================================
...
BEGIN
  ...
  -- ✅ FIXED: Use correct column name 'role_id' (not project_role_id)
  INSERT INTO project_event_roles (
    project_id,
    event_id, 
    role_id,         -- ✅ CORRECT: Use role_id column that exists
    crew_member_id,
    notes,
    is_synced,
    created_at,
  ...
  SELECT DISTINCT ON (pr.role_id)
    p_project_id,
    p_event_id,
    pr.role_id,      -- ✅ CORRECT: Use project_roles.role_id
    pr.preferred_id,
    pr.notes,
    true,
    NOW(),
